...
with tab1:
    ...
    # ── If a candidate is being chatted, show ONLY that chat ──
    if st.session_state.get("active_chat"):
        active = st.session_state.active_chat

        # Find this candidate's result
        cand_result = next((r for r in (st.session_state.results or []) if r["name"] == active), None)

        if cand_result:
            st.markdown(f"## 💬 Engaging: {active}")
            st.markdown(
                f"🎯 Match: **{cand_result['match_score']}%** &nbsp;|&nbsp; "
                f"📅 Exp: **{cand_result['experience']} yrs** &nbsp;|&nbsp; "
                f"📍 {cand_result['location']}",
                unsafe_allow_html=True
            )
            st.markdown("---")
            ...
            if step < len(questions):
                ...
                ans = st.text_input(
                    ...
                    key=f"chat_input_{name}_{step}",
                    placeholder="Type candidate's response here..."
                )

                c1, c2 = st.columns([1, 4])
                with c1:
                    if st.button("Send ➤", key=f"send_{name}_{step}"):
                        if ans.strip():
                ...
                with c2:
                    if st.button("⬅️ Back to Results", key="back_from_chat"):
                ...
                if score >= 75:
                    vt, bg, bd = "🔥 Highly Interested — Fast-track this candidate!", "#052e16", "#16a34a"
                elif score >= 50:
                    vt, bg, bd = "✅ Moderately Interested — Worth a follow-up call", "#1c1917", "#d97706"
                else:
                    vt, bg, bd = "❌ Low Interest — Consider other candidates", "#1f1215", "#dc2626"
        ...
        if st.button("🔍 Find & Rank Candidates", key="find_btn"):
            if not jd.strip():
                ...
                with st.spinner("🧠 AI parsing JD and scoring candidates..."):
                    ...
                    # Reset chat states for fresh run
        ...
        if st.session_state.results:
            ...
            for rank, r in enumerate(filtered, 1):
                ...
                with st.expander(f"{medal} {r['name']}  |  Final: {r['final_score']}%  |  Match: {r['match_score']}%  |  Interest: {r['interest_score']}%"):
                    ...
                    i_score = st.session_state.interest_scores.get(r["name"], 50)
                    if i_score > 50:
                        st.markdown(f"💬 **Interest after chat: {i_score}%** ✅")

                    st.markdown("")
                    if st.button(f"💬 Start Chat with {r['name']}", key=f"chat_btn_{r['name']}"):
...
with tab2:
    ...
    if st.button("🎯 Evaluate Now", key="eval_btn"):
        if not jd2.strip() or not resume.strip():
            ...
            with st.spinner("🧠 AI evaluating candidate..."):
                ...
                st.session_state.eval_jd      = jd2
                st.session_state.eval_resume  = resume
                st.session_state.eval_interest = None

    # ── Show persistent evaluation result ──
    if st.session_state.eval_result:
        ...
        quick_ans = st.text_input(
            "Ask: Are you open to this opportunity?",
            key="quick_ans",
            placeholder="Type candidate's response here..."
        )
        if st.button("Check Interest", key="check_interest_btn"):
            if quick_ans.strip():
        ...
        if st.session_state.eval_interest:
            i_data = st.session_state.eval_interest
            fi1, fi2 = st.columns(2)
            fi1.metric("📊 Interest Score", f"{i_data['interest']}%")
            fi2.metric("🏆 Final Score",    f"{i_data['final']}%")

        if st.button("🔄 Clear & Start Over", key="clear_eval"):
            st.session_state.eval_result  = None
            st.session_state.eval_jd      = ""
            st.session_state.eval_resume  = ""
            st.session_state.eval_interest = None
            st.rerun()
